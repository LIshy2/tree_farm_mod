...
public class SwimNodeProcessor extends NodeProcessor {
   ...
   public PathNodeType getPathNodeType(IBlockReader blockaccessIn, int x, int y, int z) {
      ...
      if (ifluidstate.isEmpty() && blockstate.allowsMovement(blockaccessIn, blockpos.func_177977_b(), PathType.WATER) && blockstate.isAir()) {
      ...
      for(int i = p_186327_1_; i < p_186327_1_ + this.entitySizeX; ++i) {
         for(int j = p_186327_2_; j < p_186327_2_ + this.entitySizeY; ++j) {
            for(int k = p_186327_3_; k < p_186327_3_ + this.entitySizeZ; ++k) {
               IFluidState ifluidstate = this.blockaccess.getFluidState(blockpos$mutable.func_181079_c(i, j, k));
               BlockState blockstate = this.blockaccess.getBlockState(blockpos$mutable.func_181079_c(i, j, k));
               if (ifluidstate.isEmpty() && blockstate.allowsMovement(this.blockaccess, blockpos$mutable.func_177977_b(), PathType.WATER) && blockstate.isAir()) {
                  return PathNodeType.BREACH;
               }
