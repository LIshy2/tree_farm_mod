      ...
      if (worldIn.isAirBlock(blockpos$mutable)) {
         if (Blocks.BAMBOO.getDefaultState().isValidPosition(worldIn, blockpos$mutable)) {
            ...
            if (rand.nextFloat() < config.probability) {
               ...
               for(int l = pos.getX() - k; l <= pos.getX() + k; ++l) {
                  for(int i1 = pos.getZ() - k; i1 <= pos.getZ() + k; ++i1) {
                     ...
                     int k1 = i1 - pos.getZ();
                     if (j1 * j1 + k1 * k1 <= k * k) {
                        blockpos$mutable1.func_181079_c(l, worldIn.getHeight(Heightmap.Type.WORLD_SURFACE, l, i1) - 1, i1);
            ...
            if (blockpos$mutable.getY() - pos.getY() >= 3) {
               ...
               worldIn.setBlockState(blockpos$mutable.func_189534_c(Direction.DOWN, 1), BAMBOO_LARGE_LEAVES, 2);
               worldIn.setBlockState(blockpos$mutable.func_189534_c(Direction.DOWN, 1), BAMBOO_SMALL_LEAVES, 2);
            }
         }
