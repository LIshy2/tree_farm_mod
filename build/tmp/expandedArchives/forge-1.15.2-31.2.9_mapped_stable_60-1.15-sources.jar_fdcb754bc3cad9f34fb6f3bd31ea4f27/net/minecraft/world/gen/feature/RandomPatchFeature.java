...
public class RandomPatchFeature extends Feature<BlockClusterFeatureConfig> {
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, BlockClusterFeatureConfig config) {
      ...
      for(int j = 0; j < config.field_227293_f_; ++j) {
         blockpos$mutable.func_189533_g(blockpos).func_196234_d(rand.nextInt(config.field_227294_g_ + 1) - rand.nextInt(config.field_227294_g_ + 1), rand.nextInt(config.field_227295_h_ + 1) - rand.nextInt(config.field_227295_h_ + 1), rand.nextInt(config.field_227296_i_ + 1) - rand.nextInt(config.field_227296_i_ + 1));
