...
public class SpringFeature extends Feature<LiquidsConfig> {
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, LiquidsConfig config) {
      if (!config.field_227366_f_.contains(worldIn.getBlockState(pos.up()).getBlock())) {
         return false;
      } else if (config.field_227363_b_ && !config.field_227366_f_.contains(worldIn.getBlockState(pos.func_177977_b()).getBlock())) {
         return false;
      } else {
         BlockState blockstate = worldIn.getBlockState(pos);
         if (!blockstate.isAir(worldIn, pos) && !config.field_227366_f_.contains(blockstate.getBlock())) {
            return false;
         } else {
            int i = 0;
            int j = 0;
            ...
            if (config.field_227366_f_.contains(worldIn.getBlockState(pos.func_177977_b()).getBlock())) {
               ++j;
            }
            ...
            if (worldIn.isAirBlock(pos.func_177977_b())) {
               ++k;
            }
