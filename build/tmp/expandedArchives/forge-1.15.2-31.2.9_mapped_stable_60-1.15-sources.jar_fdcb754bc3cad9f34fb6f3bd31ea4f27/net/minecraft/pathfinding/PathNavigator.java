...
public abstract class PathNavigator {
   ...
   public void tick() {
      ...
      if (!this.noPath()) {
         ...
         if (!this.noPath()) {
            ...
            this.entity.getMoveHelper().setMoveTo(vec3d2.x, this.world.getBlockState(blockpos.func_177977_b()).isAir() ? vec3d2.y : WalkNodeProcessor.getGroundY(this.world, blockpos), vec3d2.z, this.speed);
         }
      }
   }
   ...
   protected void pathFollow() {
      ...
      // Forge: fix MC-94054
      if (Math.abs(this.entity.func_226277_ct_() - (vec3d1.x + ((int)(this.entity.getWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.entity.func_226281_cx_() - (vec3d1.z + ((int)(this.entity.getWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.entity.func_226278_cu_() - vec3d1.y) < 1.0D) {
         this.currentPath.setCurrentPathIndex(this.currentPath.getCurrentPathIndex() + 1);
      }
