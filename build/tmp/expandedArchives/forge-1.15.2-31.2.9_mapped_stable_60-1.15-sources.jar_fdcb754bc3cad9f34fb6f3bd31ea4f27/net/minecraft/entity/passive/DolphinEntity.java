   ...
   public void tick() {
      ...
      if (!this.isAIDisabled()) {
         ...
         if (this.world.isRemote && this.isInWater() && this.getMotion().lengthSquared() > 0.03D) {
            ...
            for(int i = 0; i < 2; ++i) {
               this.world.addParticle(ParticleTypes.DOLPHIN, this.func_226277_ct_() - vec3d.x * (double)f2 + (double)f, this.func_226278_cu_() - vec3d.y, this.func_226281_cx_() - vec3d.z * (double)f2 + (double)f1, 0.0D, 0.0D, 0.0D);
               this.world.addParticle(ParticleTypes.DOLPHIN, this.func_226277_ct_() - vec3d.x * (double)f2 - (double)f, this.func_226278_cu_() - vec3d.y, this.func_226281_cx_() - vec3d.z * (double)f2 - (double)f1, 0.0D, 0.0D, 0.0D);
            }
         }
