...
public class SummonCommand {
   ...
   private static int summonEntity(CommandSource source, ResourceLocation type, Vec3d pos, CompoundNBT nbt, boolean randomizeProperties) throws CommandSyntaxException {
      ...
      if (EntityType.getKey(EntityType.LIGHTNING_BOLT).equals(type)) {
         LightningBoltEntity lightningboltentity = new LightningBoltEntity(source.func_197023_e(), pos.x, pos.y, pos.z, false);
         source.func_197023_e().addLightningBolt(lightningboltentity);
         ...
         if (entity == null) {
            ...
            if (randomizeProperties && entity instanceof MobEntity) {
               ((MobEntity)entity).onInitialSpawn(source.func_197023_e(), source.func_197023_e().getDifficultyForLocation(new BlockPos(entity)), SpawnReason.COMMAND, (ILivingEntityData)null, (CompoundNBT)null);
            }
