      ...
      if (f > 0.0F) {
         ...
         for(LivingEntity livingentity : this.world.getEntitiesWithinAABB(LivingEntity.class, this.getBoundingBox().grow(5.0D))) {
            if (livingentity != this.boostedEntity && !(this.getDistanceSq(livingentity) > 25.0D)) {
               ...
               for(int i = 0; i < 2; ++i) {
                  Vec3d vec3d1 = new Vec3d(livingentity.func_226277_ct_(), livingentity.func_226283_e_(0.5D * (double)i), livingentity.func_226281_cx_());
   ...
   public void handleStatusUpdate(byte id) {
      if (id == 17 && this.world.isRemote) {
         if (!this.func_213894_l()) {
            ...
            this.world.makeFireworks(this.func_226277_ct_(), this.func_226278_cu_(), this.func_226281_cx_(), vec3d.x, vec3d.y, vec3d.z, compoundnbt);
         }
      }
