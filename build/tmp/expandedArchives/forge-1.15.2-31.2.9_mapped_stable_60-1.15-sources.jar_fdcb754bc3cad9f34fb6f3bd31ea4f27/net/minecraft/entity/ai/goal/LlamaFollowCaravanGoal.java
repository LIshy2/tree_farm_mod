...
public class LlamaFollowCaravanGoal extends Goal {
   ...
   public void tick() {
      if (this.llama.inCaravan()) {
         ...
         float f = 2.0F;
         Vec3d vec3d = (new Vec3d(llamaentity.func_226277_ct_() - this.llama.func_226277_ct_(), llamaentity.func_226278_cu_() - this.llama.func_226278_cu_(), llamaentity.func_226281_cx_() - this.llama.func_226281_cx_())).normalize().scale(Math.max(d0 - 2.0D, 0.0D));
         this.llama.getNavigator().tryMoveToXYZ(this.llama.func_226277_ct_() + vec3d.x, this.llama.func_226278_cu_() + vec3d.y, this.llama.func_226281_cx_() + vec3d.z, this.speedModifier);
      }
   }
