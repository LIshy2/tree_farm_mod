...
public class PandaEntity extends AnimalEntity {
   ...
   private void func_213533_eu() {
      if (this.func_213559_es() % 5 == 0) {
         ...
         for(int i = 0; i < 6; ++i) {
            ...
            vec3d1 = vec3d1.add(this.func_226277_ct_(), this.func_226280_cw_() + 1.0D, this.func_226281_cx_());
   ...
   private void func_213577_ez() {
      Vec3d vec3d = this.getMotion();
      this.world.addParticle(ParticleTypes.SNEEZE, this.func_226277_ct_() - (double)(this.getWidth() + 1.0F) * 0.5D * (double)MathHelper.sin(this.renderYawOffset * ((float)Math.PI / 180F)), this.func_226280_cw_() - (double)0.1F, this.func_226281_cx_() + (double)(this.getWidth() + 1.0F) * 0.5D * (double)MathHelper.cos(this.renderYawOffset * ((float)Math.PI / 180F)), vec3d.x, 0.0D, vec3d.z);
            ...
            for(int j = 0; j < 8; ++j) {
               for(int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                  for(int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                     blockpos$mutable.func_189533_g(blockpos).func_196234_d(k, i, l);
